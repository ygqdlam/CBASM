...
def get_compromise_pseudo_btw_tea_stu(target_tea, logits_tea, target_stu, logits_stu, 
    ...
    if mtx_teacher_conflict is None:
        ...
        mtx_bool_conflict = mtx_bool_conflict_stu & mtx_teacher_conflict#当两个teacher发生冲突时会选择更可靠的那个作为结果，但是如果这个结果和学生预测的也冲突，那就忽略这部分，否则就使用教师合成的那个结果
    ...
    elif "pixel_confidence" in mode_conflict:#对于要忽略的像素中，如果学生的置信度比教师合成的置信度高，则把这部分像素以学生的预测结果的形式加回来
