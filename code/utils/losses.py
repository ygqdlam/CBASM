...
def l_correlation_cos_mean(model1, model2, linear_paras1,is_2d=False):  # fix model2
    total_loss = 0.0
    count = 0
    for name, parameters in model1.named_parameters():
        if 'conv' in name and 'weight' in name:
            if len(parameters.shape) == 5 or (is_2d and len(parameters.shape)==4):
                w1 = parameters
...
class my_DiceLoss(nn.Module):
    def __init__(self, nclass, class_weights=None, smooth=1e-5):
        super(my_DiceLoss, self).__init__()
        self.smooth = smooth
        if class_weights is None:
            # default weight is all 1
            self.class_weights = nn.Parameter(torch.ones((1, nclass)).type(torch.float32), requires_grad=False)
    ...
    def forward(self, pred, target, nclass,mask=None,d_weight=None):
        size = pred.size()
        N = size[0]
        ...
        # N x H x W
        pred_one_hot = to_one_hot(pred.type(torch.long),nclass).type(torch.float32)
        target_one_hot = to_one_hot(target.type(torch.long), nclass).type(torch.float32)
        ...
        if mask is not None:
            ...
            if d_weight is not None:
                dice = (2 * inter + self.smooth) / (union + self.smooth)
                return ((1-dice.mean(dim=1))*d_weight).mean()
        else:
            # N x C
            inter = inter.view(N, nclass, -1).sum(2)
            union = union.view(N, nclass, -1).sum(2)
