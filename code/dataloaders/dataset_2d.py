...
class BaseDataSets(Dataset):
    def __init__(
        ...
        if self.split == "train":
            ...
            with open(self._base_dir + "/temp_slices.list", "r") as f1:
                self.sample_list = f1.readlines()
            self.sample_list = [item.replace("\n", "") for item in self.sample_list]
    ...
    def __getitem__(self, idx):
        ...
        if self.split == "train" or self.split=='temp':
            h5f = h5py.File(self._data_dir + "/data/slices/{}.h5".format(case), "r")
        else:
            h5f = h5py.File(self._data_dir + "/data/{}.h5".format(case), "r")
        image = h5f["image"][:]
        label = h5f["label"][:]
