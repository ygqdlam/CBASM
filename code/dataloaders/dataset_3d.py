...
class Pancreas(Dataset):
    """ Pancreas Dataset """
    def __init__(self, base_dir=None, data_dir=None, split='train', num=None, transform=None):
    ...
    def __getitem__(self, idx):
        ...
        h5f = h5py.File(self._data_dir + "/data/" + image_name + "_norm.h5", 'r')
        image = h5f['image'][:]
        label = h5f['label'][:]
...
class LAHeart(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        h5f = h5py.File(self._data_dir+"/"+image_name+"/mri_norm2.h5", 'r')
        image = h5f['image'][:]
        label = h5f['label'][:]
        sample = {'image': image, 'label': label}
        if self.transform:
